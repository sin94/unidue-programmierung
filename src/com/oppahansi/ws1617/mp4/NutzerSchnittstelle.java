 ...
 * solve these tasks.
 *
 * https://www.youtube.com/watch?v=Y17UNx-sODE - Stream recording
 */
...
public class NutzerSchnittstelle {
        ...
        if (dienst.getHashtabelle() != null) {
            ...
            for (int i = 0; i < dienst.getHashtabelle().length; i++) {
                ...
                for (HashKnoten k = dienst.getHashtabelle()[i]; k != null; k = k.getNf()) {
                    ...
                    int a = 0;
                    for (Nachrichtenzeiger z = k.getNzKopf(); z != null; z = z.getNf())
                        a++;
    ...
    private int benutzerEingabe() {
        ...
        System.out.println("[8]  Anzahl von Nachrichten in der Nachrichtenstruktur ermitteln (Aufgabe 3b)");
        System.out.println("[9]  Durchschnittliche Anzahl an Nachrichtenzeigern pro HashKnoten ermitteln (Aufgabe 3c)");
        System.out.println("[10] Anzahl von Nachrichte mit Mindestreferenztiefe ermitteln (Aufgabe 3d)");
                ...
                for (HashKnoten hk = k; hk != null; hk = hk.getNf()) {
                    if (hk.getHashtag().equals(hashtag.substring(1))) {
                        ...
                        for (Nachrichtenzeiger z = hk.getNzKopf(); z != null; z = z.getNf()) {
                            System.out.println("\t(" + z.getNachricht().getId() + ") " + z.getNachricht().getVerfasser()
                                    + ": \"" + z.getNachricht().getInhalt() + "\"");
                            int ref = 0;
                            for (Nachricht r = z.getNachricht().getReferenz(); r != null; r = r.getReferenz()) {
                                System.out.println("\t" + tabs(++ref) + "|=>  " + "(" + r.getId() + ") "
                                        + r.getVerfasser() + ": \"" + r.getInhalt() + "\"");
                            }
                        }
                    }
                }
    ...
    private void a3a() {
        System.out.println("\tAnzahl unterschiedlicher Hashtags: " + dienst.anzahlUnterschiedlicheHashtags());
    }
    ...
    private void a3c() {
        System.out.println("\tDurchschnittliche Anzahl an Nachrichtenzeigern pro HashKnoten: "
    ...
    private void a3d() {
        ...
        System.out.println("\tAnzahl von Nachrichten mit Mindestreferenztiefe " + n + ": "
