...
public class Miniprojekt3_A2 {
    ...
    public void zeigeSkigebiet(Station[] skigebiet) {
        ...
        for(Station s : skigebiet){
            ...
            if(s.lifte!=null)for(Lift l :s.lifte) System.out.println(" "+l.name+" -> "+l.ziel.name);
            if(s.pisten!=null)for(Piste p :s.pisten) System.out.println(" "+p.name+" -> "+p.ziel.name);
        }
    ...
    public int offeneLifte(Station[] skigebiet) {
        ...
        int counter=0;
        for(Station s:skigebiet)
        if(s.lifte!=null)for(Lift l : s.lifte)if(l.status) counter++;
    ...
    public int pistenMeter(Station[] skigebiet) {
        ...
        int meter=0;
        for(Station s:skigebiet)
        if(s.pisten!=null)for(Piste p : s.pisten) meter+=p.laenge;
    ...
    public int anzahlHuetten(RElement r) {
        ...
        int huts=0;
        while (r!=null) {
            if ((r.piste != null&& r.piste.ziel.huette)||(r.lift!=null&&r.lift.ziel.huette))huts++;
            r=r.nF;
        }
    ...
    public boolean parallelePistenVon(Station s) {
        ...
        for(Piste p1:s.pisten)
            for(Piste p2:s.pisten)
                if(p1!=p2&&p1.ziel==p2.ziel) return true;
    ...
    public boolean keinLiftAbwaertsZu(Station[] skigebiet, Station s) {
        ...
        for(Station s1:skigebiet)
            if(s1.lifte!=null)
                for(Lift l:s1.lifte)if(l.ziel==s&&l.hdiff<0)return false;
        ...
        if(s.pisten!=null)
            for(Piste p:s.pisten)
                if(p.farbe=='b'&&p.ziel.lifte!=null)
                    for(Lift l:p.ziel.lifte)
                        if(l.ziel==s)return true;
        if(s.lifte!=null)
            for(Lift l:s.lifte)
                if(s.pisten!=null)
                    for(Piste p:l.ziel.pisten)
                        if(p.farbe=='b'&&p.ziel==s) return true;
        return false; // Bitte ersetzen Sie diese Zeile durch Ihre Lösung!
    }
    ...
    public boolean sindDisjunkt(RElement r1, RElement r2) {
        ...
        while (r1!=null){
            RElement tmp=r2;
            while (tmp!=null){
                if((r1.lift!=null&&r1.lift==tmp.lift)||
                        r1.piste!=null&&r1.piste==tmp.piste) return false;
                tmp=tmp.nF;
            }
            r1=r1.nF;
        }
