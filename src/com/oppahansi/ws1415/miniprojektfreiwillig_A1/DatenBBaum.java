...
public class DatenBBaum extends Datenverwaltung {
  ...
  private int anzahlEintraege(BBaumElement eintrag) {
    if (eintrag == null) {
      ...
      return 1 + anzahlEintraege(eintrag.getKleiner()) + anzahlEintraege(eintrag.getGroesserGleich());
    }
  }

  public DatenListe getAll(int id) {
    DatenListe list = new DatenListe(); //initialisiere die Liste
    this.getData(id, list, beginn);     //hole die Daten mit ID, Referenz zur liste mit übergeben + (Teil-)Baum
  ...
  private void getallData(DatenListe list, BBaumElement knoten) { //rekursive Methode
    if (knoten != null) {
      this.getallData(list, knoten.getKleiner()); //Einträge des linken (Teil-)Baums in liste einfügen
      list.einfuegen(knoten.getEintrag());            //wurzel des (Teil-)Baums einfügen
      this.getallData(list, knoten.getGroesserGleich()); //Einträge des rechten (Teil-)Baums in liste einfügen
    }//of if !=null
  }//of getallData
  ...
  public void baumBalancieren() { //rekursive Methode
    ...
    for (int i = 0; i < array.length / 2;
      ...
      tree.einfuegen(array[array.length / 2 + i]); //Elemente von Mitte array bis Ende (oder kurz vor Ende)
      tree.einfuegen(array[array.length / 2 - 1 - i]); //Element von 1 vor Mitte bis Anfang
    }//of for
    if (array.length % 2 == 1)          //Falls Array.length ungerade muss letztes Element noch eingefügt werden
    {
    ...
    beginn.setKleiner(treeleft.beginn);             //und setze diesen ausbalancierten Baum als neuen linkten Teilbaum
  ...
  private BBaumElement delData(int id, BBaumElement knoten) { //rekursive Methode
    if (knoten != null) { //Knoten vorhanden=
      ...
      {
        knoten.setKleiner(this
          .delData(id, knoten.getKleiner())); //..im linken das zu löschende Element suchen (Teilbaum mit Ergebnis ersetzen)
      }
      else //KnotenID<=id
        if (knoten.getEintrag().getProduktId() == id) { //Knoten-ID==id
          if (knoten.getGroesserGleich() == null)   //Fall 1: rechter Teilbaum null
          {
            ...
            if (knoten.getKleiner() == null) {      //Fall 2: linker Teilbaum null
              return this.delData(id, knoten.getGroesserGleich()); //einfach Rückgabe des rechten Teilbaums
            }
            else { //Fall 3: linker!=null rechter!=null
              this.getLeftitem(knoten.getGroesserGleich()).setKleiner(knoten
              ...
              knoten.setGroesserGleich(
                this.delData(id, knoten.getGroesserGleich())); //im Größergleich nach weiteren Elementen suchen
            }//of else
          return knoten.getGroesserGleich(); //RückGabe des rechten Teilbaums für Fall2+3
        }
        else //of if knoten-ID == id
        {
          knoten.setGroesserGleich(this.delData(id, knoten.getGroesserGleich())); //im größer Teilbaum suchen+löschen
        }
