...
public class DatenListe extends Datenverwaltung {
  ...
  private ListenElement fuegeEinRek(Beschreibung eintrag,
    ...
    if (jetzigesElement == null) {
      ...
      jetzigesElement.setNachfolger(fuegeEinRek(eintrag, jetzigesElement.getNachfolger(), jetzigesElement));
  ...
  public void loeschen(int id) { //ITERATIV!
    ...
    while (pointer != null) { //geht bis zum ende durch
      if (pointer.getEintrag().getProduktId() == id) { //Falls eintrag mit id gefunden
        ...
        {
          pointer.getVorgaenger().setNachfolger(pointer.getNachfolger());  //Zeiger des Vorgängers neu richten
        }
        else {
