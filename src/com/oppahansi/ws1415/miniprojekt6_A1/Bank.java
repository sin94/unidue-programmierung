...
public class Bank {
  ...
  public static final String CMD_U_EXPR = "[uU]" + "\\s+" + KTONR_EXPR + "\\s+" + KTONR_EXPR + "\\s+" + EUR_EXPR;
  ...
  public static final Class<?>[] KONTO_KLASSEN = new Class<?>[]
    {
      Sparbuch.class,
      GiroBusiness.class,
      GiroStudent.class
  ...
  public void kontoAnlegen(Class<? extends Konto> kontoKlasse, double startGuthaben)
    throws NoSuchMethodException, SecurityException, InstantiationException, IllegalAccessException, IllegalArgumentException,
    InvocationTargetException {
    Constructor<? extends Konto> constructor = kontoKlasse.getConstructor(int.class, double.class, Konto.class);
  ...
  private boolean menu() {
    ...
    System.out.printf("Willkommen in der %s!\nGeld der Bank: %s\t Datum: %s\n",
      BANK_NAME,
    ...
    System.out.printf("  d <yyyy-MM-dd>  : Datum auf das angegebene Datum erh�hen, z.B. 'd 2015-01-30'\n");
    ...
    System.out.printf("  u <k1> <k2> <e> : Ueberweisung von Konto <k1> zu Konto <k2> �ber den Betrag <e> durchfuehren.\n");
  ...
  private void menuUeberweisen(String[] parameter) throws Exception {
    ...
    if (k1.ueberweisenAuf(k2, betrag)) {
      System.out.printf("Uberweisung %d->%d (%s) erfolgreich!\n", ktNr1, ktNr2, EURO_FORMAT.format(betrag));
    }
    else {
      System.out.printf("Uberweisung %d->%d (%s) fehlgeschlagen!\n", ktNr1, ktNr2, EURO_FORMAT.format(betrag));
    }
  }
  ...
  private void menuEinzahlen(String[] args) throws Exception {
    ...
    if (k.einzahlen(betrag)) {
      ...
      System.out.printf("Einzahlen auf %d (%s) fehlgeschlagen!\n", ktNr, EURO_FORMAT.format(betrag));
    }
  }
