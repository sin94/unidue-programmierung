...
public class Miniprojekt2_A1 {
  ...
  public static void main(String[] args) {
    ...
    System.out
      .println("Der leere Saal sieht so aus (- frei, B bezahlt, R reserviert):");
    demo.debugPrint();
    System.out
      .println("Die Methode platzBelegt liefert für den existierenden Platz D 7: "
        + demo.platzBelegt('C', 7));
    System.out
      .println("Die Methode platzBelegt liefert für den nicht existierenden Platz  Q 23: "
        + demo.platzBelegt('Q', 23));
    System.out.println("Die Methode naechsteReservierungsnummer liefert: "
      + demo.naechsteReservierungsnummer());
    System.out
      .println("Die Methode festePlaetzeReservieren liefert für Reihe C Plätze 3-7 die Reservierungsnummer: "
        + demo.festePlaetzeReservieren('C', 3, 7));
    System.out
      .println("Nach der Reservierung der Plätze 3,4,5,6 und 7 in der Reihe C:");
    demo.debugPrint();
    System.out
      .println("Die Methode platzBelegt liefert für den reservierten Platz D 7: "
        + demo.platzBelegt('C', 7));
    System.out.println("Die Methode naechsteReservierungsnummer liefert: "
      + demo.naechsteReservierungsnummer());
    System.out
      .println("Die Methode festePlaetzeReservieren liefert für Reihe C Plätze 1-3 die Reservierungsnummer: "
        + demo.festePlaetzeReservieren('C', 1, 3));
    System.out
      .println("Die Methode festePlaetzeReservieren liefert für Reihe D Plätze 7-5 die Reservierungsnummer: "
        + demo.festePlaetzeReservieren('D', 7, 5));
    demo.debugPrint();
    System.out
      .println("Die Methode festePlaetzeReservieren liefert für Reihe D Plätze 1-2 die Reservierungsnummer: "
        + demo.festePlaetzeReservieren('D', 1, 2));
    demo.debugPrint();
    System.out.println("Der Anteil der freien Plätze ist: "
      + demo.anteilFreiePlaetze() * 100 + "%");
    System.out
      .println("Die Methode bezahleReservierung liefert für Reservierung 7: "
        + demo.bezahleReservierung(7));
    System.out
      .println("Die Methode bezahleReservierung liefert für Reservierung 1: "
        + demo.bezahleReservierung(1));
    demo.debugPrint();
    System.out.println("Die Methode naechsteReservierungsnummer liefert: "
      + demo.naechsteReservierungsnummer());
    System.out
      .println("Die Methode effizienteReservierung liefert für 6 Leute die Reservierungsnummer: "
        + demo.effizienteReservierung(6));
    demo.debugPrint();

    System.out
      .println("Die Methode effizienteReservierung liefert für 100 Leute die Reservierungsnummer: "
        + demo.effizienteReservierung(100));
    demo.debugPrint();
    System.out
      .println("Nach dem Aufruf der Methode neueVorstellungVorbereiten sieht der Kinosaal wieder so aus:");
    ...
    demo.debugPrint();
    System.out
      .println("Die Methode knoblauchReservierung liefert für 3 Leute die Reservierungsnummer: "
        + demo.knoblauchReservierung(3));
    demo.debugPrint();
    System.out
      .println("Die Methode knoblauchReservierung liefert für 2 Leute die Reservierungsnummer: "
  ...
  public int naechsteReservierungsnummer() {
    ...
    for (int i = 0; i < kinosaal.length; i++) {                                    // [1]
      for (int j = 0; j < kinosaal[i].length; j++) {
        ...
        if (kinosaal[i][j] >= hoechsteNummer
          && kinosaal[i][j] != Integer.MAX_VALUE) {                           // [2]
  ...
  public boolean platzBelegt(char reihe, int platz) {
    ...
    if (reihennummer < kinosaal.length && reihennummer >= 0
      && platz < kinosaal[0].length && platz >= 0) {                              // [3]
  ...
  public int festePlaetzeReservieren(char reihe, int start, int ende) {
    ...
    if (start > ende || reihennummer > kinosaal.length
      || ende > kinosaal[0].length) {                                       // [1]

    }
    else {
      ende:
      for (int i = start; i <= ende;
           i++) {                                             // ende: ist eine Sprungmarke - wird nicht mehr benutzt
      ...
      for (int i = kinosaal.length - 1; i >= 0 && buchbar == false; i--) {               // [2]
        ...
        for (int j = kinosaal[i].length - 1; j >= 0 && buchbar == false; j--) {
          ...
          if (buchbar == true) {                                             // [5]
            ...
            vergebeneNummer = festePlaetzeReservieren(reihe, start,
              ende);
