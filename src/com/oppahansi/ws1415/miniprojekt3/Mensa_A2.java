...
public class Mensa_A2 {
  ...
  Speise_A2 haupt[] = new Speise_A2[16], staerke[] = new Speise_A2[16], gemuese[] = new Speise_A2[16];
  ...
  public void anreichern() {
    for (Speise_A2 s : staerke)            //geht die St�rkespeisen durch
    {
      if (s != null)                  //abfang von NULL-Pointer!
      {
        ...
        {
          s.neueZutat(Lebensmittel.Oel, ((300 - s.getKcal()) / 9.3)); // mit �l und passender Menge anreichern
        }
      }
    }
    for (Speise_A2 g : gemuese)            //s.o.
    {
      if (g != null) {
        if (g.getKcal() < 200) {
          g.neueZutat(Lebensmittel.Margarine, ((200 - g.getKcal()) /
  ...
  public void deveganisiere() {
    Speise_A2[][] gerichte = {haupt, staerke, gemuese};   //speichert die 3 Arten von Speisen in einem Array (2-Dimensional)
    for (Speise_A2[] a : gerichte)                  //geht die Arten (Arrays) nach und nach durch
    {
      for (Speise_A2 b : a)                     //geht die Speisen der Arten nach und nach durch
      {
        if (b != null) {                     //NULL-Pointer! Speise evtl. nicht vorhanden
          b.ersetze(Lebensmittel.Kaese, Lebensmittel.Analogkaese, 1.0);            //ersetze ANALOGK�SE mit K�SE Menge (100%)
  ...
  public void rinderwahn() {
    ...
    for (Speise_A2[] a : gerichte)                  //geht Arten von Speisen durch
    {
      ...
      {
        if (a[i] != null && a[i].findeZutat(Lebensmittel.Rindfleisch)) //falls Rindfleisch gefunden wurde ...
        {
  ...
  public boolean istAusgewogen(Speise_A2 h, Speise_A2 s, Speise_A2 g) {
    double proteine = h.getPgehalt() + s.getPgehalt() + g.getPgehalt();   //speichert die Kalorien der Proteine
    double fett = h.getFgehalt() + s.getFgehalt() + g.getFgehalt();      //speichert die Kalorien des Fett
    double kohlenhydrat = h.getKgehalt() + s.getKgehalt() + g.getKgehalt();//speichert die Kalorien der Kohlenhydrate
    ...
    return (((proteine >= 0.25) && (proteine <= 0.35)) &&         //25%<=proteine<=35%
      ((kohlenhydrat >= 0.45) && (kohlenhydrat <= 0.55)) &&   //45%<=proteine<=55%
